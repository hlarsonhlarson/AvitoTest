...
func checkParams(par Parameters) error{
	if par.SortingOrder != "ASC" && par.SortingOrder != "DESC"{
		return errors.New("Invalid sorting order")
	}
	if par.SortingParameter != "price" && par.SortingParameter != "time"{
		return errors.New("Invalid sorting parameter")
	}
	return nil
}
